...
namespace CertificateGenerator
{
  ...
  internal class CLOptions
  {
    ...
    [Option('k', "keystrength", HelpText = "The Key strength of the certificate. Default is 2048.", Default = 2048)]
    public int KeyStrength { get; set; }

    [Option('a', "algorithm", HelpText = "The signature algorithm used for signing the certificate", Default = "SHA256WithRSA")]
  ...
  internal class GenericCertificateOptions : CLOptions
  {
    ...
    [Option('d', "distributionpoints", Default = new string[0], HelpText = "(Provide with Non root Certificate) An array of HTTP URIs that point to crl locations on the network\n" +
                                                  "NOTE: If none are provided, an interactive window will display asking for input")]
    public IEnumerable<string> DistributionPoints { get; set; }
  ...
  [Verb("Cert", HelpText = "Generates a leaf certificate, capable of being used for encryption, token signing and verification.\n" +
                           ...
                           "These parameters would generate a certificate with the Common Name CN=JWTSigningLeaf, signed by the InternalRootCA.\n" +
  ...
  [Verb("CA", HelpText = "Generate a root or intermediate certificate authority on the executing machine.\n" +
                         ...
                         "This parameter set would generate a root CA 0with the Common Name CN=InternalRootCA, signed by itself.\n" +
  ...
  internal class CAOptions : GenericCertificateOptions
  {
    ...
    [Option('g', "generateCRL", HelpText = "Whether or not to generate a CRL with this CA. Every CA should have its own CRL published somewhere.", Default = true)]
